...
<head>
    ...
    <style>
    	body{
			background-image: url("/eun/img/background.jpg");
		}
        .wrapper {
            position: absolute;
            top: 50%;
...
<body>
    ...
    <div class="w3-sidebar w3-bar-block w3-border-right  w3-Khaki" style="display: none" id="mysidebar">
        ...
        <span class="w3-bar-item" id="userNname">${DATA.nname}</span>
        <span class="w3-bar-item">티어 : 골드</span>
        <span class="w3-bar-item" id="userScore">최고점수 : ${DATA.score}</span>
        <input type="button" class="w3-btn w3-white w3-right w3-margin-right editBtn" value="EDIT" id="editBtn">
        </c:if>
    </div>
    <div class="w3-white w3-bar">
        <div class="w3-bar-item w3-button" id="openBtn">
            <button class="w3-button">&#9776;</button>
        </div>
        <div class="w3-bar-item w3-right">
        <c:if test="${empty SID}">
            <button class="w3-button w3-hover-white" id="loginBtn">LOGIN</button>
        </c:if>
        <c:if test="${not empty SID}">
            <!-- <button class="w3-button w3-hover-white" id="memInfo">회원정보보기</button> -->
            <button class="w3-button w3-hover-white" id="logout">LOGOUT</button>
        </c:if>
        </div>
    </div>
    <div class="wrapper">
        <div class="word w3-display-container w3-white">
            ...
            <div class="wordClass">
                <div class="w3-center">
                    <div class="w3-button w3-white" id="startBtnId">시작하기</div>
                    <!--시작하기 버튼 -->
    ...
    <div class="w3-sidebar w3-margin-top" style="width:20%;right:0">
    	<ul class="w3-ul">
    	...
    	<div class="w3-bar-item">
    		<img src="/eun/img/man.jpg" style="width:10%" class="w3-left w3-circle">
    		<span class="w3-margin-left">${list.id}</span>
    		<span class="w3-right">${list.score}점</span>
    	</div>
    	</li>
    	</c:forEach>
    	</ul>
  ...
  //사이드바
    ...
    $('#editBtn').click(function(){
		$(location).attr('href','/eun/member/userInfo.van'); 
    });
    ...
    $('#logout').click(function(){
        ...
        function timer() {
            ...
            if (timeB == 0 && timeC == -1) {
                ...
                var nowId =  "<%=session.getAttribute("SID") %>"
                var nowScore = "<%=session.getAttribute("SSCORE") %>"
                if(score > nowScore){
                		HighScore();
                }       
